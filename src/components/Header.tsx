...
const Header: React.FC<HeaderProps> = ({
  ...
  /* ---------- TOP BAR (mobile) ---------- */
  const TopBar = (
    <header className="sticky top-0 z-50 bg-white/95 backdrop-blur supports-[backdrop-filter]:bg-white/80 border-b border-gray-100 pt-[env(safe-area-inset-top)]">
      <div className="grid grid-cols-3 items-center px-3 md:px-6 py-2.5">
        <div className="flex items-center gap-1.5">
          ...
          {showBackButton && (
            <button
              onClick={() => navigate(-1)}
              className="ml-1 p-2 rounded-xl hover:bg-slate-50 active:bg-slate-100 transition
                         focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-gray-200"
              aria-label="Voltar"
              title="Voltar"
            >
        ...
        <div className="flex items-center justify-end gap-2">
          {mensagemDeSucesso && (
            <span className="text-xs md:text-sm text-emerald-600 font-medium truncate max-w-[40vw] text-right">
              {mensagemDeSucesso}
            </span>
          )}
          ...
          {onLogout && (
            <button
              onClick={onLogout}
              className="p-2 rounded-xl hover:bg-slate-50 active:bg-slate-100 transition
                         focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-gray-200"
              aria-label="Sair"
              title="Sair"
            >
      ...
      <div className="relative flex h-full flex-col">
        ...
        <nav className={`mt-2 flex flex-col gap-1.5 ${collapsed ? 'px-1' : 'px-3'}`}>
          ...
          {showBackButton && (
            <button
              onClick={() => navigate(-1)}
              className={`rounded-xl transition focus-visible:outline-none
        ...
        <div className="mt-auto px-4 py-3 flex items-center gap-2">
          {mensagemDeSucesso && !collapsed && (
            <span className="text-xs text-emerald-600 font-medium truncate max-w-[9.5rem]">
              {mensagemDeSucesso}
            </span>
          )}
          ...
          {onLogout && (
            <button
              onClick={onLogout}
              className="ml-auto p-2 rounded-xl hover:bg-slate-50 active:bg-slate-100 transition
                         focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-gray-200"
              aria-label="Sair"
              title="Sair"
            >
