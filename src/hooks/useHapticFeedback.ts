...
export function useHapticFeedback(options: HapticOptions = {}) {
  ...
  // 🛡️ Memoiza o objeto retornado para manter identidade estável entre renders.
  // Sem isto, cada render cria um objeto novo, quebrando dependências de useEffect
  // (ex.: ChatPage) e causando loop infinito de re-render ("Maximum update depth").
  return useMemo(
    () => ({
