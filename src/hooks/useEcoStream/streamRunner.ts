...
interface ControllerCancellationOptions {
  clientMessageId: string;
  controllerRef: BeginStreamParams["controllerRef"];
  activeStreamClientIdRef: BeginStreamParams["activeStreamClientIdRef"];
  streamActiveRef: BeginStreamParams["streamActiveRef"];
  streamTimersRef: BeginStreamParams["streamTimersRef"];
  tracking: MessageTrackingRefs;
  logSse: BeginStreamParams["logSse"];
}

const cancelActiveStreamIfNeeded = ({
  clientMessageId,
  controllerRef,
...
interface WatchdogSetupOptions {
  session: StreamSession;
  timers: StreamRunnerTimers;
  normalizedClientId: string;
  clientMessageId: string;
  onFirstChunk?: BeginStreamParams["onFirstChunk"];
  setDigitando: BeginStreamParams["setDigitando"];
  setIsSending: BeginStreamParams["setIsSending"];
  setErroApi: BeginStreamParams["setErroApi"];
  logSse: BeginStreamParams["logSse"];
}

interface WatchdogSetupResult {
  streamState: { fallbackRequested: boolean; firstChunkDelivered: boolean; readyReceived: boolean };
  markPromptReadyWatchdog: () => void;
  scheduleReadyTimeout: (callback: () => void, timeoutMs: number) => void;
  clearReadyTimeout: () => void;
  handleReadyEvent: () => void;
  onFirstChunkWrapped: () => void;
  markTypingActive: (source: "ready" | "first_chunk") => void;
  markTypingInactive: () => void;
  bumpFirstTokenWatchdog: () => void;
  bumpHeartbeatWatchdog: () => void;
  clearWatchdog: () => void;
  handleTypingWatchdogTimeout: () => void;
}

const setupStreamWatchdogs = ({
  session,
  timers,
  normalizedClientId,
  clientMessageId,
  ...
  setErroApi,
  logSse,
}: WatchdogSetupOptions): WatchdogSetupResult => {
  ...
  const scheduleReadyTimeout = (callback: () => void, timeoutMs: number) => {
    clearReadyTimeout();
    readyTimeoutHandle = timers.setTimeout(callback, timeoutMs);
  };
  ...
  return {
    ...
    handleTypingWatchdogTimeout,
  };
};

interface FallbackOrchestration {
  setManager: (manager: StreamFallbackManager) => void;
  getManager: () => StreamFallbackManager | null;
  clearFallbackGuardTimer: () => void;
  clearTypingWatchdog: () => void;
  beginFallback: (reason: string) => boolean;
  abortSseForFallback: (reason?: string) => void;
  startFallbackGuardTimer: (trigger: (reason: string) => void) => void;
  runJsonFallbackRequest: (options?: { reason?: string; logError?: unknown }) => Promise<boolean>;
}

const createFallbackOrchestration = (): FallbackOrchestration => {
  ...
  return {
    ...
    beginFallback: (reason) => fallbackManager?.beginFallback(reason) ?? false,
    abortSseForFallback: (reason) => {
      fallbackManager?.abortSseForFallback(reason);
    },
    startFallbackGuardTimer: (trigger) => {
      fallbackManager?.startFallbackGuardTimer(trigger);
    },
  ...
  const beginStreamInternal = (
    ...
    const {
      ...
      skipFetchInTest: skipFetchOverride,
    } = deps;
  ...
  // === streamOrchestrator.ts (parte 4/4) ===
    ...
    let pendingJsonFallback: Promise<boolean> | null = null;
    ...
    const runJsonFallbackRequest = (
      options: { reason?: string; logError?: unknown } = {},
    ): Promise<boolean> => fallbackOrchestration.runJsonFallbackRequest(options);

    cancelActiveStreamIfNeeded({
      clientMessageId,
      controllerRef,
      activeStreamClientIdRef,
      streamActiveRef,
    ...
    const fallbackOrchestration = createFallbackOrchestration();

    const clearFallbackGuardTimer = () => {
      fallbackOrchestration.clearFallbackGuardTimer();
    };

    const clearTypingWatchdog = () => {
      fallbackOrchestration.clearTypingWatchdog();
    };
    ...
    const beginFallback = (reason: string): boolean => {
      const started = fallbackOrchestration.beginFallback(reason);
      if (started) {
    ...
    const abortSseForFallback = (reason?: string) => {
      fallbackOrchestration.abortSseForFallback(reason);
    };

    const startFallbackGuardTimer = () => {
      fallbackOrchestration.startFallbackGuardTimer((reason) => {
    ...
    const diag = (event: string, payload: Record<string, unknown> = {}) => {
      try {
    ...
    let shouldSkipFetchInTest = typeof skipFetchOverride === "boolean" ? skipFetchOverride : false;
    const hasSkipOverride = typeof skipFetchOverride === "boolean";
    ...
    try {
      streamPromise = (async () => {
        ...
        const fallbackManager = new StreamFallbackManager({
          session,
          sharedContext,
          streamStats,
          guardTimeoutMs,
        ...
        fallbackOrchestration.setManager(fallbackManager);
        ...
        triggerJsonFallback = (reason: string) => {
          const fallbackPromise = tryJsonFallback(reason);
          if (reason === "fallback_guard_timeout") {
            pendingJsonFallback = fallbackPromise;
          }
        ...
        const buildRecordChain = (rawEvent?: Record<string, unknown>): Record<string, unknown>[] => {
          ...
          const payloadRecord = toRecordSafe(eventRecord?.payload);
          const metaRecord = toRecordSafe(eventRecord?.meta);
          const metadataRecord =
            toRecordSafe(payloadRecord?.metadata) ?? toRecordSafe(eventRecord?.metadata);
        ...
        const extractPayloadRecord = (rawEvent?: Record<string, unknown>) => {
          const eventRecord = toRecordSafe(rawEvent);
          return toRecordSafe(eventRecord?.payload);
        };
        ...
        session.setWatchdogTimeoutHandler((reason) => {
          if (onWatchdogTimeout) {
            onWatchdogTimeout(reason);
          }
        });
    ...
    streamPromise = streamPromise.finally(async () => {
      if (pendingJsonFallback) {
        try {
          await pendingJsonFallback;
        } catch {
          /* noop */
        }
      }
...
export const createStreamRunner = (deps: StreamRunnerDeps) => {
  const beginStream = (params: BeginStreamParams, isRetry = false) =>
    beginStreamInternal(params, deps, isRetry);
